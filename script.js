...
function renderTable(page) {
    ...
    if (filteredEntries.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="11" class="text-center">No licenses found.</td></tr>';
...
async function fetchLicenseData() {
    ...
    try {
        ...
        const tenSecondsAgoIST = currentTimeIST - 60; // 10 seconds ago in IST
        const twelveHoursAgoUTC = twelveHoursAgoIST - 19800; // Convert to UTC
        const tenSecondsAgoUTC = tenSecondsAgoIST - 19800; // Convert to UTC

        console.log('Current IST:', formatEpochTime(currentTimeIST - 19800));
        console.log('12 hours ago UTC:', twelveHoursAgoUTC, '10 seconds ago UTC:', tenSecondsAgoUTC);
        ...
        allEntries = entries.map(entry => {
            ...
            if (fields[10]) {
                generatedByCounts.set(fields[10], (generatedByCounts.get(fields[10]) || 0) + 1);
            }
            if (lastLoginTime >= twelveHoursAgoUTC && lastLoginTime <= currentTimeIST - 19800) {
                activeUsers++;
            }
            if (lastLoginTime >= tenSecondsAgoUTC && lastLoginTime <= currentTimeIST - 19800) {
                onlineUsers++;
            }
            return fields;
        ...
        Array.from(generatedByCounts.keys()).sort().forEach(user => {
        ...
        sortedSellers.forEach(({ seller, count, rank }) => {
